...
---
{schema: payload.fields map (f) -> ({key: f.key, type: describeType(f.value), nullable: f.value is Null, (example: f.value) if (f.value is String or f.value is Number), (itemCount: sizeOf(f.value)) if (f.value is Array)})}
